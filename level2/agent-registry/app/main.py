...
@app.post("/a2a/agents", status_code=201, response_model=AgentCard)
async def a2a_register_agent(card: AgentCardCreate) -> AgentCard:
    return await store.create(card)
...
@app.put("/a2a/agents/{agent_id}", response_model=AgentCard)
async def a2a_update_agent(agent_id: str, payload: AgentCardCreate) -> AgentCard:
    agent = await store.update(agent_id, payload)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent not found")
    return agent
...
@app.delete("/a2a/agents/{agent_id}", status_code=204)
async def a2a_delete_agent(agent_id: str) -> None:
    await store.delete(agent_id)


@app.post("/a2a/agents/{agent_id}/heartbeat")
async def a2a_heartbeat(agent_id: str) -> dict:
    return await heartbeat(agent_id)
